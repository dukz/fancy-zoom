...
var FancyZoomBox = {
	...
	show: function(e) {
		...
		FancyZoomBox._previous = FancyZoomBox._current;
		FancyZoomBox._current = {
			sender: element,
			content: related_div
		};

		if(element.beforeShow && element.beforeShow(element, related_div) == false) {
			FancyZoomBox._current = FancyZoomBox._previous;
			delete FancyZoomBox._previous;
			FancyZoomBox.zooming = false;
			return;
		}

		if(FancyZoomBox.zoom.style.display != 'none') {
			FancyZoomBox._removeCurrentZoomContent();
		}
		...
		new Effect.Parallel([
			...
			new Effect.Morph(FancyZoomBox.zoom, {
				...
				afterFinish: function(effect) {
					FancyZoomBox._insertCurrentZoomContent();
					FancyZoomBox.unfixBackgroundsForIE();
					FancyZoomBox.zoom_close.show();
					FancyZoomBox.zooming = false;
					if(element.afterShow) element.afterShow(element, related_div);
				}
			})
			], { duration: 0.5 });
	},
	...
	hide: function(e) {
		...
		if (FancyZoomBox._current.sender.beforeHide && FancyZoomBox._current.sender.beforeHide() == false) {
			return;
		} else {
			FancyZoomBox._previous = FancyZoomBox._current;
			delete FancyZoomBox._current;
		}
		...
		new Effect.Parallel([
				...
				new Effect.Morph(FancyZoomBox.zoom, {
					...
					afterFinish: function(effect) {
						FancyZoomBox.unfixBackgroundsForIE();
						FancyZoomBox.zooming = false;
						if(FancyZoomBox._previous.sender.afterHide) FancyZoomBox._previous.sender.afterHide();
					}
				}),
	...
	// PRIVATE METHOD: removes the related div from document.body
	// and inserts it inside the zoom content
	_insertCurrentZoomContent: function() {
		var elements = FancyZoomBox._current.content.childElements();
		elements.each(function(el) {
			el.remove();
			FancyZoomBox.zoom_content.insert(el);
		});
	},

	// PRIVATE METHOD: removes the current zoom content from the zoom div
	// and adds it into the document.body as a hidden element
	_removeCurrentZoomContent: function() {
		var elements = FancyZoomBox.zoom_content.childElements();
		elements.each(function(el) {
			el.remove();
			FancyZoomBox._previous.content.insert(el);
		});

		FancyZoomBox._previousEl = null;
	},
...
var FancyZoom = Class.create({
	initialize: function(element) {
		...
		if (this.element) {
			this.element.content_div = $(this.element.readAttribute('href').gsub(/^.*#/, '')); //Fixed cross-domain script includes (IE adds the domain to the href attribute)
			this.element.content_div.hide();

			// Add Events
			this.element.beforeShow = this.options.beforeShow;
			this.element.afterShow = this.options.afterShow;
			this.element.beforeHide = this.options.beforeHide;
			this.element.afterHide = this.options.afterHide;
