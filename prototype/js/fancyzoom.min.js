...
FancyZoomBox.fixBackgroundsForIE();},afterFinish:function(effect){FancyZoomBox._insertCurrentZoomContent();FancyZoomBox.unfixBackgroundsForIE();FancyZoomBox.zoom_close.show();FancyZoomBox.zooming=false;if(element.afterShow)element.afterShow(element,related_div);}})],{duration:0.5});},hide:function(e){e.stop();if(FancyZoomBox.zooming)return;if(FancyZoomBox._current.sender.beforeHide&&FancyZoomBox._current.sender.beforeHide()==false){return;}else{FancyZoomBox._previous=FancyZoomBox._current;delete FancyZoomBox._current;}
FancyZoomBox.zooming=true;new Effect.Parallel([new Effect.Move(FancyZoomBox.zoom,{x:FancyZoomBox.moveX*-1,y:FancyZoomBox.moveY*-1,sync:true}),new Effect.Morph(FancyZoomBox.zoom,{style:{width:'1'.px(),height:'1'.px()},sync:true,beforeStart:function(effect){FancyZoomBox.fixBackgroundsForIE();FancyZoomBox._removeCurrentZoomContent();FancyZoomBox.zoom_close.hide();},afterFinish:function(effect){FancyZoomBox.unfixBackgroundsForIE();FancyZoomBox.zooming=false;if(FancyZoomBox._previous.sender.afterHide)FancyZoomBox._previous.sender.afterHide();}}),new Effect.Fade(FancyZoomBox.zoom,{sync:true})],{duration:0.5});},_insertCurrentZoomContent:function(){var elements=FancyZoomBox._current.content.childElements();elements.each(function(el){el.remove();FancyZoomBox.zoom_content.insert(el);});},_removeCurrentZoomContent:function(){var elements=FancyZoomBox.zoom_content.childElements();elements.each(function(el){el.remove();FancyZoomBox._previous.content.insert(el);});FancyZoomBox._previousEl=null;},switchBackgroundImagesTo:function(to){FancyZoomBox.cells.each(function(td){var bg=td.getStyle('background-image').gsub(/\.(png|gif|none)\)$/,'.'+to+')');td.setStyle('background-image: '+bg);});var close_img=FancyZoomBox.zoom_close.firstDescendant();var new_img=close_img.readAttribute('src').gsub(/\.(png|gif|none)$/,'.'+to);close_img.writeAttribute('src',new_img);},fixBackgroundsForIE:function(){if(Prototype.Browser.IE7||Prototype.Browser.IE8){FancyZoomBox.switchBackgroundImagesTo('gif');}},unfixBackgroundsForIE:function(){if(Prototype.Browser.IE7||Prototype.Browser.IE8){FancyZoomBox.switchBackgroundImagesTo('png');}}};var FancyZoom=Class.create({initialize:function(element){this.options=arguments.length>1?arguments[1]:{};FancyZoomBox.init();this.element=$(element);if(this.element){this.element.content_div=$(this.element.readAttribute('href').gsub(/^.*#/,''));this.element.content_div.hide();this.element.beforeShow=this.options.beforeShow;this.element.afterShow=this.options.afterShow;this.element.beforeHide=this.options.beforeHide;this.element.afterHide=this.options.afterHide;this.element.zoom_width=this.options.width;this.element.zoom_height=this.options.height;this.element.observe('click',FancyZoomBox.show);}}});